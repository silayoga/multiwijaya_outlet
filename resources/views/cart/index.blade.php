...
<section class="py-16 md:py-24">
  <div class="max-w-3xl mx-auto px-6 md:px-10">
    <h1 class="font-display text-3xl md:text-4xl font-bold text-brand-ink tracking-tight mb-8">Your cart</h1>
    ...
    @forelse ($cart->items as $item)
      <div class="flex items-center justify-between gap-4 rounded-2xl border border-surface-border bg-white p-5 mb-4">
        <div>
          ...
          <p class="text-sm text-slate-500 mt-1">Qty {{ $item->quantity }} · {{ $item->pricingPlan?->formattedPrice() ?? 'Contact for price' }}</p>
        </div>
        <div class="flex items-center gap-4">
          ...
          <form method="POST" action="{{ route('cart.remove', $item) }}">
            @csrf
            @method('DELETE')
            <button type="submit" class="text-xs font-semibold text-slate-500 hover:text-red-600 transition-colors cursor-pointer">Remove</button>
          </form>
        </div>
      </div>
    @empty
    ...
    @if ($cart->items->isNotEmpty())
      <div class="flex items-center justify-between border-t border-surface-border pt-6 mt-6">
      ...
      <a href="{{ route('checkout.index') }}" class="mt-6 block text-center rounded-lg bg-brand-blue text-white px-6 py-3.5 text-sm font-semibold hover:opacity-90 transition-opacity">
        Proceed to checkout
      </a>
    @endif
  </div>
